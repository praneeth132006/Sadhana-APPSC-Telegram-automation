...
// Built    : 2026-09-17T08:27:44.394Z
// ==========================================================================
...
var RESERVED_SHEETS = ['Config', 'Dashboard', 'README', 'Subscribers', 'Payments', 'Support', 'Bot Settings'];

/** Tab holding one row per support ticket raised through the payment bot. */
var SUPPORT_SHEET = 'Support';

/** Support ticket columns (A..L). */
var SUPPORT_HEADERS = [
  ...
  'Username',         // E
  'Name',             // F
  'Category',         // G  payment | invite | renewal | access | other
  'Status',           // H  open | answered | closed
  ...
  'Handled By'        // L  Last admin who replied or changed the status
];
...
function doPost(e) {
  try {
    ...
    if (action === 'appendTicketMessage') {
      ...
      return jsonResponse({
        success: true,
        data: appendTicketMessage(payload.ticketId, payload.author, payload.text, payload.status, payload.handledBy)
      });
    }
...
/** Returns the Support tab, creating it if absent. */
function supportSheet() {
  var ss = book();
  var sheet = ss.getSheetByName(SUPPORT_SHEET);
  if (!sheet) {
    sheet = ss.insertSheet(SUPPORT_SHEET);
    formatSimpleSheet(sheet, SUPPORT_HEADERS, [150, 170, 170, 120, 140, 170, 100, 100, 190, 320, 480, 220]);
  }
  return sheet;
}
...
function createTicket(data) {
  return withScriptLock(function () {
    ...
    if (existingRow !== -1) {
      return rowToTicket(sheet.getRange(existingRow, 1, 1, SUPPORT_HEADERS.length).getValues()[0], existingRow);
    }
    ...
    var row = [
      ...
      String(data.bot || ''),
      message,
      appendToConversation('', conversationEntry(author, message)),
      ''
    ];
    sheet.appendRow(row);
 ...
 * @param {string} [handledBy] Admin attribution, when an admin wrote it
 * @returns {Object|null} The updated ticket, or null when it does not exist
 */
function appendTicketMessage(ticketId, author, text, status, handledBy) {
  return withScriptLock(function () {
    var sheet = supportSheet();
    var rowNumber = findTicketRow(sheet, ticketId);
    if (rowNumber === -1) return null;

    var ticket = rowToTicket(sheet.getRange(rowNumber, 1, 1, SUPPORT_HEADERS.length).getValues()[0], rowNumber);
...
/** Creates the Support and Bot Settings tabs. Safe to run repeatedly. */
function setupSupportSheets() {
  supportSheet();
  botSettingsSheet();
  book().toast('Support and Bot Settings tabs are ready.', 'Sadhana APPSC', 10);
}
