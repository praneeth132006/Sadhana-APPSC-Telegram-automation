...
// Built    : 2026-09-17T10:37:24.979Z
// ==========================================================================
...
/**
 * Support ticket columns (A..P). M..P were added later; a tab created before
 * them gets the extra headers the first time it is opened, and older rows
 * simply read those cells as blank.
 */
var SUPPORT_HEADERS = [
  ...
  'Status',             // H  open | answered | closed — see STATUS_MEANINGS
  ...
  'Admin Replies',      // M  How many answers admins have sent
  'Last Admin Reply At',// N
  'Closed At',          // O  Blank unless the ticket is closed
  'Closed By'           // P
];

/** What each ticket status means, written into the sheet as a header note. */
var STATUS_MEANINGS = {
  open: 'Waiting for an admin — the student is waiting for a reply',
  answered: 'Waiting for the student — an admin has replied',
  closed: 'Resolved — nothing left to do'
};
...
var TICKET_STATUSES = ['open', 'answered', 'closed'];
...
/** Returns the Support tab, creating it — or adding columns it lacks — as needed. */
function supportSheet() {
  var ss = book();
  var sheet = ss.getSheetByName(SUPPORT_SHEET);
  if (!sheet) {
    sheet = ss.insertSheet(SUPPORT_SHEET);
    formatSimpleSheet(sheet, SUPPORT_HEADERS,
      [150, 170, 170, 120, 140, 170, 130, 110, 190, 320, 480, 180, 90, 170, 170, 180]);
...
/** Colours the Status column and explains what each value means. */
function applySupportFormatting(sheet) {
  var rows = Math.max(sheet.getMaxRows() - 1, 1);
  var statusRange = sheet.getRange(2, 8, rows, 1); // Column H
  ...
  sheet.setConditionalFormatRules([
    rule('open', '#ffe0b2', '#e65100'),
    rule('answered', '#bbdefb', '#0d47a1'),
    rule('closed', '#c8e6c9', '#1b5e20')
  ]);
  sheet.getRange(1, 8).setNote(
    'open = ' + STATUS_MEANINGS.open + '\n' +
    'answered = ' + STATUS_MEANINGS.answered + '\n' +
...
function supportLogSheet() {
  ...
  if (!sheet) {
    ...
    formatSimpleSheet(sheet, SUPPORT_LOG_HEADERS, [180, 150, 120, 200, 80, 160, 420, 100]);
  }
  return sheet;
}
...
function rowToTicket(row, rowNumber) {
  return {
    ...
    status: String(row[7] || '').trim().toLowerCase(),
 ...
 *
 * The id is minted by the bot, so a retried delivery of the same message
 * returns the existing row instead of opening a second ticket.
 *
 * @param {Object} data ticket_id, telegram_id, username, name, category, bot, message
...
function createTicket(data) {
  return withScriptLock(function () {
    ...
    return rowToTicket(row, sheet.getLastRow());
  });
}

/**
 * appendTicketMessage — adds one message to a ticket's thread.
 *
 * @param {string} ticketId
 * @param {string} author Who wrote it, as it should read in the thread
 * @param {string} text
 * @param {string} [status] New status, when the message changes it
...
function appendTicketMessage(ticketId, author, text, status, handledBy, logAction) {
  return withScriptLock(function () {
    ...
    var nextStatus = TICKET_STATUSES.indexOf(String(status || '').toLowerCase()) !== -1
    ...
    sheet.getRange(rowNumber, 8).setValue(nextStatus);
    sheet.getRange(rowNumber, 10).setValue(String(text));
    sheet.getRange(rowNumber, 11).setValue(appendToConversation(ticket.conversation, conversationEntry(author, text)));
    if (isAdmin) {
      sheet.getRange(rowNumber, 12).setValue(String(handledBy));
      sheet.getRange(rowNumber, 13).setValue(ticket.admin_replies + 1);
    ...
    writeSupportLog(ticketId, ticket.telegram_id, isAdmin ? handledBy : author, isAdmin ? 'admin' : 'student',
      logAction || (isAdmin ? 'admin_reply' : 'student_message'), text, nextStatus);
    if (ticket.status === 'closed' && nextStatus !== 'closed') {
      writeSupportLog(ticketId, ticket.telegram_id, isAdmin ? handledBy : author, isAdmin ? 'admin' : 'student',
        'reopened', 'Reopened by a new message', nextStatus);
    }
...
/** Changes a ticket's status. Returns null when the ticket does not exist. */
function setTicketStatus(ticketId, status, handledBy) {
  var next = String(status || '').toLowerCase();
  if (TICKET_STATUSES.indexOf(next) === -1) {
  ...
  return withScriptLock(function () {
    ...
    var before = readTicketRow(sheet, rowNumber);
    var now = istNow();
    sheet.getRange(rowNumber, 3).setValue(now);
    sheet.getRange(rowNumber, 8).setValue(next);
    if (handledBy) sheet.getRange(rowNumber, 12).setValue(String(handledBy));
    if (next === 'closed') {
      sheet.getRange(rowNumber, 15).setValue(now);
      sheet.getRange(rowNumber, 16).setValue(String(handledBy || ''));
    } else {
      sheet.getRange(rowNumber, 15).setValue('');
      sheet.getRange(rowNumber, 16).setValue('');
    }

    var action = next === 'closed' ? 'closed'
      : (before.status === 'closed' ? 'reopened' : 'status_' + next);
    writeSupportLog(ticketId, before.telegram_id, handledBy || '', handledBy ? 'admin' : '',
      action, before.status + ' → ' + next, next);
    return readTicketRow(sheet, rowNumber);
  });
}
 ...
 * such as checking a payment or an invite that could not be sent.
 *
...
function logTicketEvent(ticketId, who, role, action, details) {
  return withScriptLock(function () {
    ...
    if (rowNumber === -1) return null;
    var ticket = readTicketRow(sheet, rowNumber);
    sheet.getRange(rowNumber, 3).setValue(istNow());
    if (role === 'admin' && who) sheet.getRange(rowNumber, 12).setValue(String(who));
    writeSupportLog(ticketId, ticket.telegram_id, who, role, action, details, ticket.status);
    return readTicketRow(sheet, rowNumber);
  });
}
...
function ticketLog(ticketId) {
  ...
  for (var i = 0; i < values.length; i++) {
    ...
    out.push({
      ...
      status_after: String(values[i][7] || '')
    });
  }
...
/**
 * listTickets — newest first, with per-status counts for the dashboard tiles.
 *
 * @param {Object} params status, search, telegramId, page, pageSize
 */
function listTickets(params) {
  var sheet = supportSheet();
  var lastRow = sheet.getLastRow();
  var counts = { open: 0, answered: 0, closed: 0, total: 0 };
...
function recordRedemption(data) {
  ...
  return withScriptLock(function () {
    ...
    log.appendRow([
      istNow(), code, String(data.telegram_id || ''), String(data.username || ''), String(data.group || ''),
